...
from training.image_to_mask.mask_to_dataframe import get_logits_from_image, decode_all_leads
from training.image_to_mask.metrics import plot_leads_compare_dynamic, ecg_snr_db, fit_gain_offset, align_by_xcorr, \
...
if __name__ == "__main__":
    ...
    if inference:
        ...
        snr_map = {}
        for scan_type in allowed_types:
            print(f"Processing ECG ID: {ecg_id} Type: {scan_type}")
            ...
            img_preprocessed = preprocess_for_model(img_path, final_h=config.H_T, final_w=config.W_T, rectify=True)
            ...
            if not is_test_dataset:
                ...
                snr_map[(ecg_id, scan_type)] = snr_ecg_db
                if debug:
        ...
        # SNR summary:
        # SNR summary (pretty table)
        print("\nSNR summary:\n")

        rows = []
        for (ecg_id, scan_type), snr_val in sorted(
                snr_map.items(),
            ...
            rows.append(
                (
                    str(ecg_id),
                    str(scan_type),
                    str(config.libelle_type.get(scan_type, "")),
                    float(snr_val),
                )
            )
