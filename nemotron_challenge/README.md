...
Latest tracked public result: `exp13_trace_v2_aug25k_10k_inout_r25_ep1` checkpoint 78 and final step 313 both scored `0.57`. Earlier `exp12_mamba_trace_v2_aug25k_5k_inout_vo_r4_ep1` checkpoint/final 105 scored `0.58`, and `exp12_mamba_trace_v2_aug25k_inout_qkvo_r4_ep1` scored `0.54` despite the strongest recent broad local generated eval at `36/64 = 0.5625`. These remain below `exp05_trace_occam_r4_inout` checkpoint 144 at `0.59` and below the earlier `00-raw-1024` baseline at about `0.62`, so the current v2 trace format is not yet sufficient.

Sanity-test remark: all tracked experiments have failed the three public `test.csv` sanity rows without exception.
