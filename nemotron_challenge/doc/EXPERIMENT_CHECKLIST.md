...
Latest local preparation is `2026-05-27`. Start here before reading the lower historical notes.
...
| Current conclusion | Natural cipher alignment and v2 broad traces both improved output discipline, but neither taught enough hard-family rule inference. The next dataset needs ordered cipher alignment, bit rule-search before execution, and broad all-family rehearsal. |
| Next active task | If making one last capacity/target-surface check before new data, run the non-fused 4-bit exp12 notebook targeting only `in_proj/out_proj`; otherwise design the next broad hard-family dataset with compact ordered cipher traces and bit candidate-rule search traces. |
...
- [x] `exp12_mamba_trace_v2_aug25k_inout_qkvo_r4_ep1`
  ...
  - Public score: `0.54`.
  - Archive: `data/outputs/submissions/2026-05-27_exp12_mamba_trace_v2_aug25k_inout_qkvo_r4_ep1_ready/`.
...
- [ ] `exp13_mamba_trace_v2_aug25k_qplusc_qkvo_r4_ep1` `RISK`
  - Purpose: test whether completion-only loss made teacher-forced metrics too easy by masking the puzzle/question tokens.
  - Notebook: `notebooks/10_colab_mamba_trace_v2_aug25k_question_loss_train_and_submit.ipynb`.
