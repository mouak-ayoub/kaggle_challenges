...
Latest local preparation is `2026-06-03`. Start here before reading the lower historical notes.
...
| Next active task | Treat capacity-only v2 scaling as rejected for now. Design the next method around new data construction: offline rejection sampling first, then a broad hard-family dataset with compact ordered cipher traces, bit candidate-rule search traces, and all-family rehearsal only if acceptance/verification coverage is real. |
